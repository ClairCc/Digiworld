...
(function($) {
	...
	$.fn.FancyFileUpload = function(options) {
		...
		var AddFile = function(uploads, e, data) {
			...
			{
				inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('Subir')).click(StartUpload));
				inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('Detener Subida')).click(StartUpload));
			}

			inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('aria-label', Translate('Remover archivo')).click(RemoveFile));
			inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('aria-label', Translate('Remover archivo')).click(RemoveFile));
		...
		var UploadFailed = function(e, data) {
			...
			{
				data.ff_info.fileinfo.text(FormatStr(Translate('{0} | Error de red, volviendo a intentar en un momento ...'), data.ff_info.displayfilesize));
			...
			{
				// Set the error info.
				if (data.errorThrown === 'abort')  data.ff_info.errors.push(Translate('La carga fue cancelada.'));
		...
		var UploadDone = function(e, data) {
			...
			{
				if (typeof(data.result.error) !== 'string')  data.result.error = Translate('El servidor indicó que la carga no se realizó correctamente. No hay información adicional disponible.');
			...
			{
				// Set the status.
				data.ff_info.inforow.find('.ff_fileupload_fileinfo').text(data.ff_info.displayfilesize + ' | ' + Translate('Carga completada'));

				// Hide the progress bar.
				data.ff_info.inforow.find('.ff_fileupload_progress_background').addClass('ff_fileupload_hidden');

				// Alter remove buttons.
				data.ff_info.inforow.find('button.ff_fileupload_remove_file').attr('aria-label', Translate('Quitar de la lista'));
			}
		};
